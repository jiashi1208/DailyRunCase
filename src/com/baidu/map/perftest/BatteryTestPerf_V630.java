...
public class BatteryTestPerf_V630 extends ActivityInstrumentationTestCase2 {
	...
	/**
	 * test_SearchBusRoute() author:shijia 7.3.0修改了【到这去】 步骤。 必须要选中去bus。
	 */
	...
	public void test_searchBankPlace() throws Exception {
		...
		mToolKit.Sleep(PAUSE_TIME);
		mToolKit.SearchForText("西单", "到这去");
		solo.waitForText("搜周边", 1, PAUSE_TIME * 5);
		mToolKit.clickOnText("搜周边", true, PAUSE_TIME);

		solo.scrollDown();
		solo.waitForText("银行", 1, PAUSE_TIME * 2);
		mToolKit.clickOnText("银行", true, PAUSE_TIME);
		solo.waitForText("品牌", 1, PAUSE_TIME * 10);
		solo.waitForText("到这去", 1, PAUSE_TIME * 2);
		solo.clickOnText("到这去", 0);
		solo.waitForText("推荐路线", 1, PAUSE_TIME * 6);
		Log.i("shijia", "搜索银行 点击 驾车图标");
		mToolKit.sleep(PAUSE_TIME * 2);
		mToolKit.clickOnViewById("layout_topbar_middle_car", PAUSE_TIME * 6);
		solo.waitForText("导航", 1, PAUSE_TIME * 6);
		mToolKit.clickOnText("偏好", true, PAUSE_TIME);
		solo.waitForText("确认", 1, PAUSE_TIME);
		mToolKit.clickOnText("高速优先", true, PAUSE_TIME);
	...
	public void test_searchSurround1() throws Exception {
		...
		solo.waitForText("搜周边", 1, PAUSE_TIME * 2);
		mToolKit.clickOnText("搜周边", true, PAUSE_TIME);
		solo.waitForText("中餐", 1, PAUSE_TIME * 2);
		mToolKit.clickOnText("中餐", true, 0);
		solo.waitForText("地图", 1, PAUSE_TIME * 3);
	...
	public void test_searchSurround2() throws Exception {
		...
		solo.waitForText("搜周边", 1, PAUSE_TIME * 3);
		mToolKit.clickOnText("搜周边", true, PAUSE_TIME);
		solo.waitForText("小吃快餐", 1, PAUSE_TIME * 3);
		mToolKit.clickOnText("小吃快餐", true, PAUSE_TIME);
		solo.waitForText("地图", 1, PAUSE_TIME);
		mToolKit.clickOnText("地图", true, PAUSE_TIME * 2);
		mToolKit.BackToMapMain();
	}
	...
	public void test_searchSurround3() throws Exception {
		...
		// mToolKit.clickOnScreen(RIDConfigV630.ID_ROUTE_FOOT_BUTTON,
		// PAUSE_TIME*2);
		mToolKit.Sleep(PAUSE_TIME * 5);
	...
	public void TestBattery() throws Exception {
		for (int count = 0; count < 2; count++) {
			...
			LinkedHashMap<String, Boolean> allCase = new LinkedHashMap<String, Boolean>();

			allCase.put("test_searchBusRoute()", false);
			allCase.put("test_POISearch2()", false);
			allCase.put("test_POISearch1()", false);
			allCase.put("test_searchBankPlace()", false);
			allCase.put("testLayer()", false);
			allCase.put("test_searchSurround1()", false);
			allCase.put("test_searchSurround2()", false);
			allCase.put("test_CrossCity()", false);
			for (int x = 0; x < 3; x++) {
				Iterator<String> it = allCase.keySet().iterator();
				while (it.hasNext()) {
					if (allCase.containsValue(false)) {
						String caseName = it.next();
						Log.i("shijia", "caseName:" + caseName);
						if (!allCase.get(caseName)) {
							try {
								runTestCase(caseName);
								allCase.put(caseName, true);
								Log.i("shijia", "caseName:" + caseName + " Passed");
							} catch (Throwable e) {
								if (x != 2) {
									allCase.put(caseName, false);
									Log.i("shijia", "caseName:" + caseName + " Failed");
									Log.i("shijia", "failed" + caseName + ".png");
									solo.takeScreenshot("failed" + caseName + ".png");
									mToolKit.Sleep(PAUSE_TIME * 2);
									try {
										mToolKit.BackToMapMain();
									} catch (Exception e1) {
									...
									Log.i("TAG", "final failed");
									throw new Exception("Case failed during 3 times:", e);
								}

							}
						}
					} else {
						Log.i("shijia", "TestResult: All cases Passed");
						break;
					}
	...
	private void runTestCase(String caseName) throws Exception {
		if (caseName.equalsIgnoreCase("test_POISearch1()")) {
			test_POISearch1();
		} else if (caseName.equalsIgnoreCase("test_POISearch2()")) {
			test_POISearch2();
		} else if (caseName.equalsIgnoreCase("test_searchBusRoute()")) {
			test_searchBusRoute();
		} else if (caseName.equalsIgnoreCase("test_searchBankPlace()")) {
			test_searchBankPlace();
		} else if (caseName.equalsIgnoreCase("testLayer()")) {
			testLayer();
		} else if (caseName.equalsIgnoreCase("test_searchSurround1()")) {
			test_searchSurround1();
		} else if (caseName.equalsIgnoreCase("test_searchSurround2()")) {
			test_searchSurround2();
		} else if (caseName.equalsIgnoreCase("test_CrossCity()")) {
			test_CrossCity();
		}
	}
	...
	public void testLayer() throws Exception {
		solo.waitForText("我的", 1, PAUSE_TIME);
		solo.waitForText("路线", 1, PAUSE_TIME);
		solo.waitForText("附近", 1, PAUSE_TIME);
	...
	public void ZoomIn(int deltaLv) {
		for (int i = 0; i < deltaLv; i++) {
			// mToolKit.clickOnViewById(RIDConfigV630.ID_ZOOM_IN_BUTTON,
			// PAUSE_TIME);
	...
	public void ZoomOut(int deltaLv) {
		for (int i = 0; i < deltaLv; i++) {
			// mToolKit.clickOnViewById(RIDConfigV630.ID_ZOOM_OUT_BUTTON,
			// PAUSE_TIME);
